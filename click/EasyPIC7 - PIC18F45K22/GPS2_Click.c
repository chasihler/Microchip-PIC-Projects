 ...
 * Revision History:
     20121218:
       - initial release (DO)
 * Description:
     This example demonstrates the functionality of the GPS2 Click board.
 ...
 * Test configuration:
     ...
     Ext. Modules:    GPS2 Click - ac:GPS2_Click
                      http://www.mikroe.com/click/gps2/
     SW:              mikroC PRO for PIC
                      http://www.mikroe.com/mikroc/pic/
 * NOTES:
     - Place GPS2 Click board in the mikroBUS socket 1 on the EasyPIC7 board.
     - Turn on GLCD backlight switch SW.4.6. (board specific)
 */

#include "World Map.h"

sbit power_ON at LATA.B2;
sbit power_ON_Direction at TRISA.B2;
sbit WAKEUP at RC0_bit;
sbit WAKEUP_Direction at TRISC.B0;
sbit RST at RE1_bit;
...
void main() {
  ...
  ANSELD = 0;                      //
  ANSELE = 0;
  power_ON_Direction = 0;          //
  WAKEUP_Direction = 1;            // Setup GSM2 click control pins
  RST_Direction = 0;               //
  RST = 1;                         // Enable module
  ...
  UART1_Init(4800);                // Initialize UART module at 4800

  power_ON = 1;
  Delay_ms(400);

  while (WAKEUP != 1){             // If module is in hibernation
    power_ON = 0;                  //
    Delay_1ms();                   // Issue the wakeup signal
    Delay_1ms();                   //
    power_ON = 1;                  // wait for next WAKEUP checkup
    Delay_ms(321);
  }
