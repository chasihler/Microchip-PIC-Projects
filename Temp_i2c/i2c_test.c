...
//**************************************************************************************
void Send_i2c_CByte_R(void)
{
    PIR1bits.SSP1IF=0;          // clear SSP interrupt bit

    SSPBUF = 0xEF;             // send the control byte (90)
    while(!PIR1bits.SSP1IF)     // Wait for interrupt flag to go high indicating transmission is complete
        {
            LATCbits.LATC0 = 0;
...
{
    PIR1bits.SSP1IF=0;          // clear SSP interrupt bit
    SSP1BUF = 0x90;  // send the control byte
    while(!PIR1bits.SSP1IF)    // Wait for interrupt flag to go high indicating transmission is complete
        {
         // place to add a breakpoint if needed
        }
...
void Send_I2C_StartBit(void)
{
    ...
    //if (PIR2bits.BCL1IF)      //collision detect debugging
    //        LATCbits.LATC2 = 1;
...
//    Send_I2C_Data(word_address);            // send word address
    ...
    {
    LATAbits.LATA2 = 0;                 //something
    }
    LATAbits.LATA2 = 0;                 //something                                    // now we read the data coming back from the eeprom
