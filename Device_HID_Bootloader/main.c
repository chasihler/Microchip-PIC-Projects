...
void main(void)
{   
    //Need to make sure sw2 can be used as a digital input pin.
    ...
    #elif defined(LF25K50_DEMO)
        unsigned char ANSELASave;
        ANSELASave = ANSELA;
        ANSELAbits.ANSA3 = 0;
        TRISAbits.TRISA3 = 1;   //No need to explicitly do this since reset state is = 1 already.
    #else
    ...
    {
        ClrWdt();
        USBTasks(); // Need to call USBTasks() periodically									// it handles SETUP packets needed for enumeration
        BlinkUSBStatus();
...
//	to it in io_cfg.h.    
    ...
    //these may be application specific!
        ...
        *((unsigned char*)0xFB5) = 0x90;  //Enable active clock tuning for USB operation
        ...
        //ACTCONbits.ACTSRC = 1;  // Tune to USB clock source
        //ACTCONbits.ACTEN = 1;
