...
void determine_drive_by_sonar(void)
{
    ...
    if (pulse_time < 0xAF && pulse_time > 0x30)
    {
        SENSORbits.waiting=1;
        DRIVEbits.stop=1;
    }

    if (pulse_time < 0x30 && !DRIVEbits.estop && pulse_time > 0x01)
    {
        SENSORbits.waiting=0;
        DRIVEbits.rev=1;
    }
...
void set_drive(void)
{
    if  (DRIVEbits.estop)
        motor_stop();
    else if (DRIVEbits.left_rev && !DRIVEbits.rev)
        motor_left_rev_quarter();
    else if (DRIVEbits.right_rev && !DRIVEbits.rev)
